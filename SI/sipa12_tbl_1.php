...
*-----------------------------------------------------------------------------*/
  ...
  $conn->debug = false;
   ...
   $aColumns = array('modulo', 'nombre', 'estado' );
   $IndexColumn = "modulo";
   $Table = "modulos_web";
  ...
  $Sql="SELECT modulo, nombre, estado
FROM (
  SELECT modulo, nombre, estado, rnum
  FROM (
    SELECT modulo, nombre, estado,
...
WHERE rnum <= $Fin AND rnum > $Inicio   ";
  ...
  foreach($rs as $rcon){
    ...
    $aRow=array();
    $aRow[] ='<span class="linkCont">'.$rcon["modulo"].'</span>';
    $aRow[] = $rcon["nombre"];
	  $aRow[] = $rcon["estado"];
	  $aRow[] = "<a href='?f=sipa12&_g=add&ided=".$rcon["modulo"]."' class='btn btn-sm btn-warning'>+ Editar</a>";
    $aOutput['aaData'][] = $aRow;
